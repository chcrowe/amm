    ...
    <div>
      {provider && amm ? (
        <div>
          ...
          <Table id="transactions" striped bordered hover>
            <thead>
              <tr>
                <th>Transaction Hash</th>
                <th>Token (Provided)</th>
            ...
            <tbody>
              {chart.swaps && chart.swaps.map((swap, index) => (
                <tr key={index}>
                  ...
                  <td className='tableAmount'>{ethers.utils.formatUnits(swap.args.tokenGiveAmount.toString(), 'ether')}</td>
                  <td>{swap.args.tokenGet === tokens[0].address ? symbols[0] : symbols[1]}</td>
                  <td className='tableAmount'>{parseFloat(ethers.utils.formatUnits(swap.args.tokenGetAmount.toString(), 'ether')).toFixed(18)}</td>
